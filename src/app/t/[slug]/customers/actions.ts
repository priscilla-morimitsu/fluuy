...
export async function updateCustomerTagAction(slug: string, id: string, name: string): Promise<CustomerTagActionResult> {
  try {
    ...
    const existing = await prisma.customerTag.findFirst({
      ...
      select: { id: true, color: true, _count: { select: { assignments: true } } },
    });
    ...
    return {
      ok: true,
      tag: { id: tag.id, name: tag.name, slug: tag.slug, color: tag.color, status: tag.status, customerCount: existing._count.assignments },
    };
  } catch (err) {
    return fail(err);
  }
