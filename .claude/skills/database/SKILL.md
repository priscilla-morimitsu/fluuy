---
name: database
description: Database access patterns, query safety, migration discipline, and data integrity requirements. Apply to files matching: lib/**/*.ts, prisma/**, drizzle/**, migrations/**.
---
...
- Every tenant-scoped table needs an index on `tenant_id`; add a GIN index on `custom_data` when it is filtered/queried (see the multi-tenant rule in `CLAUDE.md`).
